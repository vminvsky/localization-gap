...
async def main(original_dir, steering_dir, output_dir, num_samples=5, compare_steering=False, model="gpt-4o-2024-11-20"):
    ...
    plt.title('Faithfulness Win Rates by Language and Method', fontsize=16)
    plt.xlabel('Language', fontsize=14)
    plt.ylabel('Win Rate', fontsize=14)
    plt.ylim(0, 1)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'faithfulness_comparison.png'))
    plt.close()
    
    # Print summary
    print("Faithfulness Win Rates:")
    for lang in win_rates:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--original_dir", type=str, default="../data/open_ended_generation/per_model/Gemma_2_9b")
